...
class Server:
    ...
    def __init__(self, ksize=20, alpha=3, node_id: bytes | None = None, storage: PersistentStorage | None = None):
        ...
        self.routing = RoutingTable(self.ksize, self.node)
        FileSystemProtocol.init(self.routing, self.storage)
    ...
    def listen(self, port, interface='0.0.0.0'):
        ...
        self.node.port = port
        t = ThreadedServer(ServerSessionService, port=port, hostname=interface)
        t.start()
    ...
    def bootstrappable_neighbors(self):
        ...
        """
        neighbors = FileSystemProtocol.router.find_neighbors(self.node)
        return [tuple(n)[-2:] for n in neighbors]

    def bootstrap(self, addrs: list[tuple[str, str]]):
        """
        ...
        spider = NodeSpiderCrawl(self.node, nodes,
                                 self.ksize, self.alpha)
        return spider.find()

    def bootstrap_node(self, addr: tuple[str, str]):
        response = None
        with ServerSession(addr[0], addr[1]) as conn:
            response = conn.rpc_ping(addr, self.node.id)
    ...
    def set(self, key, value, apply_hash_to_key=True):
        """
        ...
        if apply_hash_to_key:
            key = digest(key)
        return self.set_digest(key, value)

    def set_digest(self, dkey: bytes, value):
        """
